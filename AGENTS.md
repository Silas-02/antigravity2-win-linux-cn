...
- Never run `node localization_engine.js`, `install.sh`, `uninstall.sh`, any batch or command launcher, or any equivalent command that installs, injects, restores, repacks, or otherwise modifies the user's Antigravity client during development. Read-only extraction of a confirmed official package into a separate temporary directory is allowed for verification; never write the result into the application directory.
