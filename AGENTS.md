...
- The current localization release is **v2.11.0**, targeting the official **Antigravity v2.11.0** client. The current version dictionary is `dicts/v2.11.0.json`; keep these values, the release date, and the support statement in `README.md` consistent.
...
- For dictionary or engine changes, add or update focused cases in `tests/renderer-regression.js` for the changed behavior, including singular/plural, fragmented or intermediate states, protected zones, repeat processing, dynamic updates, sibling isolation, and preserved interactive nodes as applicable. Then run the repository-owned non-installing verifier: `node scripts/verify.js`. Do not replace it with ad hoc commands or restate its internal checks here; inspect every failure and reported skip.
